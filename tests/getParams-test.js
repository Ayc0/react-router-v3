...
import { getParams } from "../modules/PatternUtils";
...
describe("getParams", function () {
  ...
  describe("when a pattern has dynamic segments", function () {
    ...
    describe("and the path matches", function () {
      it("returns an object with the params", function () {
        expect(getParams(pattern, "/comments/abc.js/edit")).toEqual({
          id: "abc",
          ext: "js",
        });
      });
    ...
    describe("and the pattern is optional", function () {
      ...
      describe("and the path matches with supplied param", function () {
        it("returns an object with the params", function () {
          expect(getParams(pattern, "/comments/123/edit")).toEqual({
            id: "123",
          });
        });
      });
      ...
      describe("and the path matches without supplied param", function () {
        it("returns an object with an undefined param", function () {
          expect(getParams(pattern, "/comments//edit")).toEqual({
            id: undefined,
          });
        });
      });
    ...
    describe("and the path matches with a segment containing a .", function () {
      it("returns an object with the params", function () {
        expect(getParams(pattern, "/comments/foo.bar/edit")).toEqual({
          id: "foo",
          ext: "bar",
        });
      });
  ...
  describe("when a pattern has dynamic segments and characters that have special URL encoding", function () {
    ...
    describe("and the path matches", function () {
      it("returns an object with the params", function () {
        expect(getParams(pattern, "/comments/abc/edit now")).toEqual({
          id: "abc",
        });
      });
    });
    ...
    describe("and the path contains multiple special URL encoded characters", function () {
      ...
      describe("and the path matches", function () {
        it("returns the correctly decoded characters", function () {
          expect(getParams(pattern, "/foo/%7Bfoo%24bar")).toEqual({
            component: "{foo$bar",
          });
        });
      });
  ...
  describe("when a pattern has a *", function () {
    describe("and the path matches", function () {
      it("returns an object with the params", function () {
        expect(getParams("/files/*", "/files/my/photo.jpg")).toEqual({
          splat: "my/photo.jpg",
        });
        expect(getParams("/files/*", "/files/my/photo.jpg.zip")).toEqual({
          splat: "my/photo.jpg.zip",
        });
        expect(getParams("/files/*.jpg", "/files/my/photo.jpg")).toEqual({
          splat: "my/photo",
        });
        expect(getParams("/files/*.jpg", "/files/my/new%0Aline.jpg")).toEqual({
          splat: "my/new\nline",
        });
      });
    });
  ...
  describe("when a param has dots", function () {
    ...
    describe("and the path matches", function () {
      it("returns an object with the params", function () {
        expect(getParams(pattern, "/foo/with/foo.app")).toEqual({
          query: "foo",
          domain: "foo.app",
        });
        expect(getParams(pattern, "/foo.ap/with/foo")).toEqual({
          query: "foo.ap",
          domain: "foo",
        });
        expect(getParams(pattern, "/foo.ap/with/foo.app")).toEqual({
          query: "foo.ap",
          domain: "foo.app",
        });
      });
