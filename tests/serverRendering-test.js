...
import RouterContext from "../modules/RouterContext";
...
describe("server rendering", function () {
  class App extends Component {
    render() {
      return (
        <div className="App">
          <h1>App</h1>
          <Link to="/about" activeClassName="about-is-active">
            About
          </Link>{" "}
          <Link to="/dashboard" activeClassName="dashboard-is-active">
            Dashboard
          </Link>
  ...
  class Async extends Component {
    render() {
      return (
        <div className="Async">
          <h1>Async</h1>
          <Link to="/async" activeClassName="async-is-active">
            Link
          </Link>
        </div>
      );
  ...
  it("accepts a custom history", function (done) {
    ...
    match(
      { history, routes, location: "/dashboard" },
      function (error, redirectLocation, renderProps) {
        const string = renderToString(<RouterContext {...renderProps} />);
        expect(string).toMatch(/The Dashboard/);
  ...
  it("sends null values when no routes match", function (done) {
    match(
      { routes, location: "/no-match" },
      function (error, redirectLocation, state) {
  ...
  it("accepts a basename option", function (done) {
    match(
      { routes, location: "/dashboard", basename: "/nasebame" },
      function (error, redirectLocation, renderProps) {
        const string = renderToString(<RouterContext {...renderProps} />);
    ...
    it("should match for asynchronous route", (done) => {
      match(
        ...
        (error, redirectLocation, renderProps) => {
          ...
          match(
            { history: createMemoryHistory("/async"), routes },
            (error, redirectLocation, renderProps) => {
              const browserString = renderToStaticMarkup(
                <Router {...renderProps} />
              );
