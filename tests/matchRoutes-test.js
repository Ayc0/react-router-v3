...
import { createRoutes } from "../modules/RouteUtils";
...
describe("matchRoutes", function () {
  ...
  function describeRoutes() {
    ...
    describe("when the location matches a nested route with multiple splat params", function () {
      it("matches the correct routes and params", function (done) {
        matchRoutes(
          routes,
          createLocation("/files/a/b/c.jpg"),
          function (error, match) {
            expect(match).toExist();
    ...
    describe("when the location matches a deeply nested route with params and hash", function () {
      it("matches the correct routes and params", function (done) {
        matchRoutes(
          routes,
          createLocation("/users/5/abc#about"),
          function (error, match) {
            expect(match).toExist();
            expect(match.routes).toEqual([
              RootRoute,
      ...
      routes.forEach(function (route) {
        ...
        if (childRoutes) {
          ...
          route.getChildRoutes = () =>
            new Promise((resolve) => resolve(childRoutes));
        ...
        if (indexRoute) {
          ...
          route.getIndexRoute = () =>
            new Promise((resolve) => resolve(indexRoute));
        }
      });
    }
  ...
  describe("an asynchronous JSX route config", function () {
    ...
    beforeEach(function () {
      getChildRoutes = expect
        .createSpy()
        .andCall(function (partialNextState, callback) {
          setTimeout(function () {
            callback(null, <Route path=":userId" />);
          });
        });

      getIndexRoute = expect.createSpy().andCall(function (location, callback) {
        setTimeout(function () {
